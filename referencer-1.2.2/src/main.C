...
int main (int argc, char **argv)
{
	...
	/* we should use SNAP_USER_DATA here */
	/*if (getenv("SNAP")){
	    Glib::ustring homePlugins;
	     homePlugins = Glib::ustring(getenv("SNAP_USER_DATA")) + Glib::ustring("/.referencer/plugins");
	}else{*/ 
   ...
   /* } */
	...
	/* Systemwide */
	Glib::ustring installedPlugins;
	if (getenv("SNAP")){
	    installedPlugins = Glib::ustring(getenv("SNAP")) + Glib::ustring("/lib/referencer/") ;
	    DEBUG (String::ucompose ("installedPlugins is set to %1", installedPlugins));
	}else{
	    installedPlugins = PLUGINDIR;
        DEBUG (String::ucompose ("installedPlugins is set to %1", installedPlugins));
    }
	...
	_global_plugins->scan(installedPlugins);
